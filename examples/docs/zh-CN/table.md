...
:::demo 在`el-table`中设置`sortable`属性即可实现以该列为基准的排序，接受一个`Boolean`，默认为`false`。
```html
  <template>
    <el-table
      :data="tableData"
...
| transposable | 对应列是否可以通过拖动改变位置，实际未换位置，通过监听 header-transposed 换位 （需要在 el-table 上设置 border 属性为真） | boolean | — | true |
